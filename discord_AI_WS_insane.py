...
from sinks.whisper_sink import (
    WhisperSink,
)  # User whisper to transcribe audio and outputs to TTS
...
async def whisper_message(queue: asyncio.Queue):
    while True:
        ...
        if response is None:
            ...
            obj = {"type": type , "name": username.display_name, "text": text}
...
async def join(ctx):
    ...
    if ctx.author.voice:
        ...
        whisper_sink = iwhisper_sink.iWhisperSink(
            queue,
            loop,
            data_length=25000,
            quiet_phrase_timeout=1,
            mid_sentence_multiplier=1.35,
...
async def get_username(user_id):
    # cache and optimize
    if user_id in cacheNames:
        return cacheNames[user_id]
    name = await client.fetch_user(user_id)
    cacheNames[user_id] = name
    return name
